import React from "react";
import "../styles/Category.css";
function Category() {
  return (
    <div className="category-section">
      <h2 className="category-title">Latest Posts</h2>
      <div className="filter-section">
        <img
          style={{ width: "30px" }}
          className="funnel-icon"
          src="data:image/png;base64,"
          alt="Filter icon"
        />
        <p>Filter by Category</p>
      </div>
      <div className="button-group">
        <button className="active-btn btn">All</button>
        <button className="btn btn-primary">Artificial Intelligence</button>
        <button className="btn btn-primary">Cloud Computing</button>
        <button className="btn btn-primary">DevOps</button>
        <button className="btn btn-primary">Programming Languages</button>
        <button className="btn btn-primary">
          Mobile Application Development
        </button>
        <button className="btn btn-primary">Technology and Tools</button>
        <button className="btn btn-primary">Get a Jon in a Tech Company</button>
        <button className="btn btn-primary">Others</button>
      </div>
    </div>
  );
}

export default Category;
